...
Node* DFS(Node* initial, int* cont){
  ...
  while (top(pila) != NULL){
    ...
    Node *vecino;
    while(top(adj_list) != NULL){ // Si hay vecinos
      vecino = top(adj_list); // Obtengo el primer nodo
      pop(adj_list);  // Lo elimino de la lista
      push(pila, vecino); // Agrego el vecino a la pila
    }
  }
  return NULL;
}
